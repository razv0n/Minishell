...
/*   Updated: 2025/06/20 10:33:20 by mfahmi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	env_to_double_pointer(t_info *info)
{
	...
	info->env = ft_malloc(sizeof(char *) * (lenght + 1), FIRST_P, FREE);
	while (head)
	{
...
char	*ft_getenv(char *nm_varible, t_list *env)
{
	...
	while (env)
	{
		if (!ft_strncmp(nm_varible, env->content, lenght) && *(env->content
				+ lenght) == '=')
		{
			expand = ft_substr(env->content, lenght + 1, ft_strlen(env->content)
					- lenght + 1);
			break ;
		}
		env = env->next;
